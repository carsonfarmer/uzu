...
from inputs import HEADER,NAMES
...
def run_exact(data,scores,residual,workers=128,interleave=True,return_parts=False,rows=32):
    assert rows in (8,16,32) and 0<workers<=160
    ...
    out,d=down(inputs=[h,data[2],*data[9:12],scores,a,residual],template=[('ROWS',rows)],
        grid=(16384,1,1),threadgroup=(rows*8,1,1),
