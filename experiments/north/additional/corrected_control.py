...
a=p.parse_args()
assert a.runs%10==0, 'Five-way order requires complete ten-round balanced cycles'
...
with output.open('w') as f:
    ...
    sources=[Path(__file__).resolve(),HERE/'gpu_run.py',HERE/'layers.py',HERE/'prep.py',HERE/'prep.h',HERE.parent/'reference.py',HERE.parent/'full_layer/benchmark_decode.py']+[HERE.parent/'quantized'/n for n in ('patch.py','exact.py','kernel.h','kernels.py')]
    ...
    modes={'exact_host':('exact','host'),'exact_joint':('exact','joint'),'exact_async':('exact','async'),'prepared_async':('prepared','async'),'original_async':('original','async')}
