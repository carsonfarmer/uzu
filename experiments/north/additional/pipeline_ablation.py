"""Separate evaluation-boundary savings from host submission overlap.
...
with output.open('w') as f:
    ...
    sources=[Path(__file__).resolve(),HERE/'gpu_run.py',HERE/'layers.py',HERE/'prep.py',HERE/'prep.h',HERE.parent/'reference.py']+[HERE.parent/'quantized'/n for n in ('patch.py','exact.py','kernel.h','kernels.py')]
    ...
    def generate(ids,path,asynchronous,joint=False,retain=False):
        ...
        for step in range(a.tokens-1):
            ...
            if asynchronous:
                ...
                mx.eval(logits,current) if joint else mx.eval(logits)
    ...
    modes={'exact_sync':('exact',False,False),'exact_joint':('exact',False,True),'exact_async':('exact',True,True),'prepared_async':('prepared',True,True)}
    for label in a.prompts:
        ...
        expected=None
        if a.check:
            expected,ref,_,_=generate(ids,'original',False,retain=True)
