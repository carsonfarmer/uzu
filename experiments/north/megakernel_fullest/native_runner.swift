...
let lib=try device.makeLibrary(URL:URL(fileURLWithPath:manifest.library))
...
try emit(["kind":"native_provenance","device":device.name,"maxThreads":pipeline.maxTotalThreadsPerThreadgroup,"threadWidth":pipeline.threadExecutionWidth,"staticThreadgroupBytes":pipeline.staticThreadgroupMemoryLength,"scope":"GPU command-buffer duration, includes workspace fill when dispatching. Exported primitive, not model decode."])
for rep in -3..<manifest.repetitions {
    let order=rep%2==0 ? [false,true] : [true,false]
    for fillOnly in order {
        ...
        if !fillOnly {
            ...
            compute.dispatchThreadgroups(MTLSize(width:manifest.workers,height:1,depth:1),threadsPerThreadgroup:MTLSize(width:256,height:1,depth:1))
            compute.endEncoding()
        }
        ...
        try emit(["kind":"native_timing","repetition":rep,"warmup":rep<0,"fillOnly":fillOnly,"wallSeconds":wall,"gpuSeconds":gpu,"gpuTimestampsAvailable":command.gpuStartTime>0 && gpu>0])
        if !fillOnly && rep==manifest.repetitions-1 {
            try Data(bytes:output.contents(),count:manifest.outputBytes).write(to:URL(fileURLWithPath:manifest.output))
        }
    }
}
