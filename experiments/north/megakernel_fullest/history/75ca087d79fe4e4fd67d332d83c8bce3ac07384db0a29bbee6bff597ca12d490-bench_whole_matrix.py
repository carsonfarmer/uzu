...
p.add_argument('--workers',type=int,default=32);p.add_argument("--schedulers",nargs="+",choices=["scan","affinity","progress","prefetch_early","prefetch_late"],default=["affinity","progress","prefetch_late","prefetch_early"])
    ...
    for name in modes:
        ...
        for step,(r,v) in enumerate(zip(refs,got)):
            ...
            if name not in ('prepared','phase'):
                ...
                if name=='prefetch_early':assert min(row['early_weights'])>0 and row['early_weights']==row['consumed_weights'],row
