...
for workers in a.workers:
    ...
    row = {
        ...
        "early_weights": counters[SIZE:].reshape(a.layers,512)[:,496:500].sum(axis=0).tolist(),
        "consumed_weights": counters[SIZE:].reshape(a.layers,512)[:,500:504].sum(axis=0).tolist(),
...
sources = [p for p in Path(__file__).resolve().parent.iterdir() if p.suffix in ('.py','.h')]+[p for folder in ('whole_pass','full_layer','quantized','persistent') for p in Path(__file__).resolve().parents[1].joinpath(folder).iterdir() if p.suffix in ('.py','.h')]
result = {
