...
p.add_argument('--workers',type=int,nargs='+',default=[32,64,96]);p.add_argument("--schedulers",nargs="+",choices=["scan","affinity","progress","prefetch_early","prefetch_late"],default=["progress"])
    ...
    for name in modes:
        ...
        for step,(r,v) in enumerate(zip(refs,got)):
            ...
            if name!='prepared' and not name.startswith('phase@'):
                ...
                if name.startswith('prefetch_early@'):assert min(row['early_weights'])>0 and row['early_weights']==row['consumed_weights'],row
