...
p.add_argument('--workers',type=int,nargs='+',default=[32,64,96]);p.add_argument("--schedulers",nargs="+",choices=["scan","affinity","progress","prefetch_early","prefetch_late","window_early","window_late"],default=["progress"])
a=p.parse_args();assert a.runs%(2*(1+len(a.workers)*(1+len(a.schedulers))))==0
...
with output.open('w') as f:
    ...
    def generate(mode,retain=False):
        workers=None
        if "@" in mode:
            mode,workers=mode.split("@");workers=int(workers)
        ...
        for step in range(a.tokens-1):
            if mode not in ('prepared','original'):
                ...
                if mode!='phase':got=fine(weights,h,keys,values,position,workers=workers,scheduler=mode)
                else:got=run_whole_pass(weights,h,keys,values,position,workers=workers,schedule='queue',do_head=True,do_prefix=True)
    ...
    modes=['prepared']+[f'{name}@{workers}' for workers in a.workers for name in ['phase']+a.schedulers]
    for name in modes:
        ...
        for step,(r,v) in enumerate(zip(refs,got)):
            ...
            if name!='prepared' and not name.startswith('phase@'):
                ...
                if name.startswith('prefetch_early@'):assert min(row['early_weights'])>0 and all(e<=c for e,c in zip(row['early_weights'],row['consumed_weights'])),row
