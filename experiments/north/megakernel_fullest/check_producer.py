...
p=argparse.ArgumentParser();p.add_argument('--output',required=True);p.add_argument('--layers',type=int,nargs='+',default=[7]);p.add_argument("--leader",action="store_true");a=p.parse_args()
...
with out.open('w') as f:
 ...
 for index in a.layers:
  ...
  for name,module in [('q',layer.self_attn.q_proj),('k',layer.self_attn.k_proj),('v',layer.self_attn.v_proj),('router',layer.mlp.gate)]:
   ...
   for depth in (1,2):
    got,norm,errors=run(x,module.weight,layer.input_layernorm.weight,router=name=='router',depth=depth,leader=a.leader);mx.eval(got,norm,errors)
