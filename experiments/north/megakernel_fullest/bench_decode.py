"""Matched async control and balanced four-way ready-tail comparison.
...
from cross_layer import path as cross_path
...
with output.open('w') as f:
    ...
    sources=sorted(set([q for folder in (HERE,HERE.parent/'additional',HERE.parent/'quantized') for q in folder.iterdir() if q.suffix in ('.py','.h','.swift')]+[HERE.parent/'reference.py',HERE.parent/'full_layer/tail_prep.py',HERE.parent/'persistent/down.h']))
    ...
    modes={name:(name,'async') for name in ('prepared','ready_direct','ready_late','ready_early')}
