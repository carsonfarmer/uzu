...
class DecodeBranch(nn.Module):
    def __init__(self,original,mode,chunk=64,workers=128,rows=32):
        ...
        assert mode in ('native_compiled','custom','exact','scheduled','prefetch','staged')
...
def variants(model,chunk=64,workers=128,modes=('native_compiled','custom'),rows=32,schedule_workers=64):
    ...
    return {'original':original,**{
        mode:[original[0]]+[DecodeBranch(layer,mode,chunk,schedule_workers if mode in ('scheduled','prefetch','staged') else workers,rows) for layer in original[1:]]
        for mode in modes}}
