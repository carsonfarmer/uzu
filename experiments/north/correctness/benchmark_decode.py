...
p.add_argument('--teacher-steps',type=int,default=0)
p.add_argument('--workers',type=int,default=160)
p.add_argument('--rows',type=int,default=16)
p.add_argument('--modes',nargs='+',choices=['native_compiled','exact','scheduled','prefetch','staged','fast'],default=['native_compiled','exact'])
p.add_argument('--prompts',nargs='+',default=['short','long','rust'])
p.add_argument('--output',default='experiments/north/correctness/results/benchmark-exact-v2.jsonl')
a=p.parse_args()
model,config=load();paths=variants(model,workers=a.workers,rows=a.rows,modes=tuple(a.modes))
...
prior={r['prompt']:r['token_ids'] for r in map(json.loads,(ROOT/'experiments/north/correctness/results/full-exact-v1.jsonl').read_text().splitlines()) if r['kind']=='completion'}
...
save(dict(kind='provenance',device=mx.device_info(),mlx=mx.__version__,args=vars(a),
    ...
    source_sha256={name:hashlib.sha256((ROOT/'experiments/north/quantized'/name).read_bytes()).hexdigest() for name in ['exact.py','patch.py','kernels.py','kernel.h']},
    ...
    specialization=dict(workers=a.workers,rows=a.rows,interleave=True,patched_layers=list(range(1,49))),
    ...
    for label in a.prompts:
        ...
        for rep in range(-1,a.runs):
            ...
            for mode in order:
                ...
                assert row['token_ids']==prior[label][:len(row['token_ids'])],(label,mode,'token mismatch')
